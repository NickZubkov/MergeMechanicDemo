...
План выполнен целиком — открытых задач по нему нет. Остаются только вещи, сознательно вынесенные за скоуп (раздел «Что осталось за рамками» в плане: тесты, сохранение состояния, анимации, звук) и два тач-риска к будущему APK из отчёта ревью:

- **`IsPointerOverGameObject()` без `fingerId`** в `DragInputController` — на телефоне не погасит драг, начатый на кнопке.
- Проверить компоновку на реальном вертикальном экране: резерв под UI считается от кнопки и должен отработать сам, но глазами это ещё не смотрели.
